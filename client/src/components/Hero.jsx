        ...
        <div className={`transition-all duration-300 pt-15 px-4 sm:px-20 xl:px-32 relative inline-flex flex-col w-full justify-center min-h-screen ${darkMode ? '' : 'bg-[url(/gradientBackground.png)] bg-cover bg-no-repeat'}`}>
            ...
            <div className='text-center mb-6'>
                <h1 className={`transition-all duration-300 text-3xl sm:text-5xl md:text-6xl 2xl:text-7xl font-semibold mx-auto leading-[1.2] ${darkMode ? 'text-gray-300' : ''}`}>
                    {darkMode ?
                        <><span className="transition-all duration-300 animate-gradient bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 bg-clip-text text-transparent font-bold">Create amazing content <br /> with{' '}AI tools
                        </span>
                        </>
                        :
                        <><span>Create amazing content <br /> with{' '}</span>
                            <span className="transition-all duration-300 animate-gradient bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 bg-clip-text text-transparent font-bold">
                                AI tools
                            </span>
                        </>
                    }
                </h1>
                <p className={`transition-all duration-300 mt-4 max-w-xs sm:max-w-lg 2xl:max-w-xl m-auto max-sm:text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600 '}`}>
            ...
            {/* Dummy Users */}
            {
                trustedCount ?
                    (
                        <div className={`flex items-center gap-4 mt-8 mx-auto text-gray-600 ${darkMode ? 'text-secondary' : ''}`}>
